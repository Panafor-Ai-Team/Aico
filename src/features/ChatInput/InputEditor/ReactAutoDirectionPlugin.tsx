...
/**
 * Host editor surface this plugin needs. Narrowed to what it calls so the
 * wiring can be tested against a stub — the bug this guards against lives in
 * the wiring, not the rule.
 */
export interface AutoDirectionHost {
  getLexicalEditor: () => LexicalEditor | null;
  off: (event: 'initialized', handler: (editor: LexicalEditor) => void) => void;
  once: (event: 'initialized', handler: (editor: LexicalEditor) => void) => void;
}

/**
 * Registers the direction transforms, now or as soon as the inner editor
 * exists. Returns the teardown.
 *
 * Returning early when `getLexicalEditor()` is null is NOT safe: the effect
 * that calls this has a stable dependency, so it never runs again and the
 * plugin stays dead for the life of the composer. React StrictMode hides that
 * in development by invoking effects twice — the second pass finds the editor —
 * which is why an early return looks fine in `dev` and ships broken.
 */
export const attachAutoDirection = (
  editor: AutoDirectionHost,
  getUiDirection: () => 'ltr' | 'rtl',
): (() => void) => {
  let teardowns: (() => void)[] = [];
  ...
  const attach = (lexicalEditor: LexicalEditor) => {
    ...
    const sync = () => $syncRootDirection(getUiDirection());
 ...
 * direction described the previous keystroke: the first character typed rendered
 * with the old direction and only snapped into place on the second. Transforms
 * run inside the same update, before reconciliation.
 */
const ReactAutoDirectionPlugin: FC = () => {
  const [editor] = useLexicalComposerContext();

  useEffect(() => attachAutoDirection(editor as AutoDirectionHost, getDocumentDirection), [editor]);
