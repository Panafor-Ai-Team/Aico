...
/** @deprecated No-op — catalog prices are no longer marked up on the serve path. */
export const __resetUsageMultiplierCache = () => undefined;
...
export class AiInfraRepos {
  ...
  /**
   * Managed catalog prices are served as **raw** upstream rates. The platform
   * margin is taken at top-up (fewer π per dollar), not by marking up picker
   * prices — so users see π 1:1 with CVC capacity.
   *
   * Kept as a named step so call sites stay explicit about the managed path.
   */
  private withUsageMultiplier = async (
    models: AiProviderModelListItem[],
    _providerId: string,
  ): Promise<AiProviderModelListItem[]> => models;
