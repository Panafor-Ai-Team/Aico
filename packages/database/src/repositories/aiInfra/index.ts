...
export class AiInfraRepos {
   ...
   *
   * Per-model coefficients come only from the upstream catalog (synced every
   * 6h); the platform markup is the one multiplier applied here, matching
   * `resolveManagedPricingContext` so the picker and per-message cost agree.
   */
  private withUsageMultiplier = async (
    ...
    const bp = await this.resolveUsageMultiplierBp(configId);
    return models.map((model) => ({
      ...model,
      pricing: applyPricingMultiplier(model.pricing, bp),
    }));
  };
