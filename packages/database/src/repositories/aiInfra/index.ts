...
/** Shared across repository instances — the multiplier is platform-global. */
const USAGE_MULTIPLIER_TTL_MS = 60_000;
let usageMultiplierCache: { at: number; bp: number } | null = null;

/** Test seam: drop the cached multiplier so a change is picked up immediately. */
export const __resetUsageMultiplierCache = () => {
  usageMultiplierCache = null;
};
...
export class AiInfraRepos {
  ...
  /**
   * AICO-180 platform usage multiplier, cached in-process so a model-list
   * request does not add a config round-trip per call.
   */
  private resolveUsageMultiplierBp = async (): Promise<number> => {
    const now = Date.now();
    if (usageMultiplierCache && now - usageMultiplierCache.at < USAGE_MULTIPLIER_TTL_MS) {
      return usageMultiplierCache.bp;
    }
    const bp = await new AicoBillingModel(this.db).getUsageMultiplierBp();
    usageMultiplierCache = { at: now, bp };
    return bp;
  };
  ...
  private withUsageMultiplier = async (
    ...
    const bp = await this.resolveUsageMultiplierBp();
  ...
  private fetchBuiltinModels = async (
    ...
    // Managed Aico traffic is resold OpenRouter capacity — every price shown for
    // it carries the platform markup, on every branch below including the
    // static model-bank fallback.
    const isManagedProvider = providerId === 'openrouter' || providerId === BRANDING_PROVIDER;
    try {
      ...
      if (providerId === 'openrouter') {
        ...
        if ((await catalog.count()) > 0) {
          return this.withUsageMultiplier(await catalog.listAsProviderModels());
        }
      ...
      const builtin = (presetList as AIChatModelCard[]).map<AiProviderModelListItem>((m) => ({
        ...m,
        enabled: m.enabled || false,
        source: AiModelSourceEnum.Builtin,
      }));

      // Reached on first setup or when OpenRouter is unreachable. Without the
      // markup here the fallback would silently serve raw resale prices.
      return isManagedProvider ? this.withUsageMultiplier(builtin) : builtin;
    } catch (error) {
      console.error(error);
